    ...
    <div className="space-y-4">
      {feedbackHistory.length > 0 ? (
        feedbackHistory.map((update) => (
          <Card key={update.id} className="shadow-sm border rounded-xl bg-secondary/20">
            <CardContent className="p-4 space-y-2">
              <div className="flex items-start gap-4">
                <div className="flex-shrink-0 mt-1">
                  {update.type === 'Program Feedback' && (
                ...
                <div className="flex-1 space-y-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <h4 className="font-semibold text-base md:text-lg overflow-hidden whitespace-nowrap text-ellipsis">
                      {update.title}
                    </h4>
