...
export default function HorizontalCalendar({
  selectedDate, setSelectedDate, schedule
}: { selectedDate: Date, setSelectedDate: (d: Date) => void, schedule: ScheduledTask[] }) {
      ...
      {weekDates.map(date => {
        ...
        return (
          <button key={date.toString()} onClick={() => setSelectedDate(date)}
            className={cn("p-2 rounded-xl min-w-[60px] text-center", isSameDay(date, selectedDate) ? "bg-emerald-500 text-white" : "bg-white")}>
            <div className="text-xs">{format(date,"EEE")}</div>
            <div className="text-lg font-bold">{format(date,"d")}</div>
            <div className="flex justify-center gap-1 mt-1">
              {tasks.map(t => <div key={t.id} className={`w-2 h-2 rounded-full ${hasMissed?"bg-red-400":"bg-green-500"}`}></div>)}
            </div>
          </button>
        );
      })}
