...
const AlertItem = ({ emoji, emojiBg, title, description, onDismiss }) => (
  <div className="p-4 px-6 flex items-center gap-4 group hover:bg-slate-50/50 transition-colors">
    ...
    <div className="flex items-center gap-2">
        {/* Proceed Button */}
        <button className="p-2 rounded-full text-slate-500 hover:bg-orange-100 hover:text-orange-600 transition-colors">
            <ArrowRight className="w-5 h-5" />
        </button>
        {/* Dismiss Button */}
        <button onClick={onDismiss} className="p-2 rounded-full text-slate-400 hover:bg-slate-200 hover:text-slate-600 transition-colors">
            <X className="w-5 h-5" />
        </button>
    </div>
  </div>
