...
const ClientHeader: React.FC<ClientHeaderProps> = ({ client }) => {
  ...
  return (
    <motion.div
      className="bg-card/70 backdrop-blur-md rounded-2xl border border-border shadow-sm p-4 sm:p-6 overflow-hidden flex flex-col gap-6"
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center w-full justify-between gap-4">
        ...
        <div className="flex items-center gap-4 flex-1">
          <motion.img
            className="h-16 w-16 sm:h-20 sm:w-20 rounded-full object-cover border border-border shadow-sm"
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-1">
              ...
              {hasNewFeedback && (
                <Badge className="rounded-full px-2 py-0.5 text-xs font-medium bg-yellow-500/10 text-yellow-600 border-yellow-500/20">
                  💬 New Feedback
                </Badge>
              )}
            </div>
        ...
        <div className="flex flex-row gap-2 sm:gap-3">
          <Button className="rounded-full text-sm h-10 px-4 flex items-center">
            <MessageCircle className="h-4 w-4 mr-1.5" />
            Feedback
          </Button>
          <Button
            variant="outline"
            className="rounded-full text-sm h-10 px-4 flex items-center"
          >
      ...
      <div className="grid grid-cols-3 gap-3 sm:gap-6 w-full">
        {[
          ...
          <div
            key={i}
            className="flex flex-col items-center justify-center bg-muted/30 rounded-xl p-3 sm:p-4 shadow-sm hover:shadow transition"
          >
            <stat.icon className="h-5 w-5 text-green-500 mb-1.5" />
