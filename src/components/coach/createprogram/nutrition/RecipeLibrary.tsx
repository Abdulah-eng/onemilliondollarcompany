  ...
  return (
    <div className="space-y-3 sm:space-y-4 lg:space-y-2 h-full flex flex-col p-3 sm:p-4 lg:p-2">
      ...
      <div className="space-y-3 sm:space-y-4 lg:space-y-5">
        ...
        {hasActiveFilters && (
          <div className="text-sm sm:text-xs lg:text-xs text-muted-foreground px-1">
          ...
          {searchResults.length > 0 ? (
            searchResults.map((recipe) => (
              <motion.div
                ...
                className="group flex items-center gap-3 p-4 rounded-xl border border-border bg-card shadow-sm hover:shadow-xl hover:border-primary/20 transition-all duration-300 cursor-pointer hover:-translate-y-1 min-h-[80px] touch-manipulation active:scale-[0.98] hover:bg-card/80"
              >
                {/* Image Placeholder */}
                <div className="w-14 h-14 flex-shrink-0 rounded-lg overflow-hidden bg-muted flex items-center justify-center text-xl text-primary/70 group-hover:scale-105 transition-transform duration-300">
                ...
                <div className="flex-1 flex flex-col justify-center min-w-0 gap-1">
                  <div className="flex items-start justify-between mb-1">
                    <h4 className="text-base font-semibold text-foreground leading-tight pr-2 group-hover:text-primary transition-colors duration-200 truncate">
                      {recipe.name}
                    </h4>
                    <span
                      className="text-xs px-2 py-0.5 rounded-full font-medium capitalize flex items-center gap-1 shrink-0 shadow-sm bg-blue-100 text-blue-700"
                    >
                      {recipe.mealTypes[0].replace('-', ' ')}
                    </span>
                  </div>
                  <p className="text-sm text-muted-foreground leading-relaxed truncate">
                    {recipe.description || 'Nutritious meal option.'}
                  </p>
            ...
            <div className="p-8 text-center text-muted-foreground">
              <div className="text-base font-medium">
                {searchQuery.length > 2 ? 'No recipes found' : 'Search recipes...'}
              </div>
            </div>
          )}
        </AnimatePresence>
