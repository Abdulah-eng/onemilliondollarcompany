...
const NutritionSummary: React.FC<NutritionSummaryProps> = ({ data, activeDay }) => {
  ...
  return (
    ...
    <div className="p-3 md:p-4 space-y-4 h-full"> 
      ...
      {/* Total Macros Card - Simplified and Stable Layout */}
      <div className="p-3 rounded-xl bg-gradient-to-br from-card to-background shadow-lg border border-border/50 flex flex-col gap-3">
        ...
        {/* Total Calories Block - Simplified structure, removed inner p-3 container */}
        ...
        {/* Macro Breakdown - Optimized for narrow column */}
        <div className="grid grid-cols-3 gap-1">
            {[
                ...
                // Reduced padding and height for better fit
                <div key={macro.label} className='flex flex-col items-center justify-center p-1 rounded-md bg-secondary/30 h-[65px]'>
                    ...
                    <span className='text-[10px] text-muted-foreground leading-none mt-1 uppercase'>{macro.label}</span>
                </div>
            ))}
        </div>
      </div>
      ...
      {/* Content Area - Adjusted max height for stability */}
        ...
        <AnimatePresence mode="popLayout">
          {allItems.length > 0 ? (
            Object.keys(data).map((sectionKey) => {
                ...
                const items = data[section] || []; // Use data directly
                    ...
                    <div key={section} className='space-y-2 pb-3 border-b border-border/70 last:border-b-0'> 
                        ...
                        {items.map((item) => (
                            <motion.div
                                ...
                                // Reduced padding to p-2 for narrow card
                                className="flex flex-col p-2 bg-background rounded-lg border border-border/50 shadow-sm"
                            >
