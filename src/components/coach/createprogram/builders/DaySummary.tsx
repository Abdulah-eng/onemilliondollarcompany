...
  items: WorkoutDayItem[]; // items already filtered by the parent (FitnessBuilder)
  activeDay: string; // ISO date string
}

const DaySummary: React.FC<DaySummaryProps> = ({ items, activeDay }) => {
  // Since FitnessBuilder now passes only the items for the activeDay, 
  // the filtering is simplified, but we still use the passed 'items'
