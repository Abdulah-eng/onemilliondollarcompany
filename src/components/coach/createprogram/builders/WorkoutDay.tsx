...
const WorkoutDay: React.FC<WorkoutDayProps> = ({ day, items, onItemsChange }) => {
  ...
  return (
    <Card className="p-4 md:p-6 space-y-4 h-full">
      ...
      <Droppable droppableId="workout-day">
        {(provided) => (
          <div
            ref={provided.innerRef}
            {...provided.droppableProps}
            className="space-y-4 min-h-[100px] border-2 border-dashed border-gray-200 rounded-xl p-4"
          >
            <AnimatePresence>
                ...
                <Draggable key={item.id} draggableId={item.id} index={itemIndex}>
                  {(provided) => (
                    <motion.div
                      ...
                      layout
                    >
                      <div className="flex items-center justify-between gap-2 mb-2" {...provided.dragHandleProps}>
            ...
            {items.length === 0 && (
              <div className="text-center text-muted-foreground py-12 border-dashed rounded-lg">
                Drag exercises here to start building your workout.
              </div>
            )}
          </div>
        )}
