...
import { format } from 'date-fns'; // ADD format
...
  // CHANGE: Use ISO date string for activeDay and workoutData keys
  const initialActiveDay = format(new Date(), 'yyyy-MM-dd');
  const [activeDay, setActiveDay] = useState(initialActiveDay);
  // CHANGE: workoutData keys are now ISO date strings
...
      reps: [0],
      comment: '',
      date: activeDay, // ADD date field to item, which is the activeDay ISO string
    };
...
  // NEW: Compute the list of dates that have exercises (for DateCircles indicator)
...
          <WorkoutDay
            // CHANGE: Format the activeDay ISO string back to a day name for display in WorkoutDay title
            day={format(new Date(activeDay), 'EEEE')} 
