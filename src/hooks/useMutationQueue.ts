  ...
  const mutate = useCallback(
    async (
      ...
      const mutationId = await queueMutation(type, table, payload, {
        ...
        queryKey: options?.queryKey || options?.invalidateQueries?.[0],
...
export function useTableMutations(table: string) {
  ...
  const insert = useCallback(
    async (payload: any, options?: { queryKey?: string[]; invalidateQueries?: string[][] }) => {
  ...
  const update = useCallback(
    async (
      payload: any,
      filters: Record<string, any>,
      options?: { queryKey?: string[]; invalidateQueries?: string[][] }
    ) => {
  ...
  const remove = useCallback(
    async (filters: Record<string, any>, options?: { queryKey?: string[]; invalidateQueries?: string[][] }) => {
