  ...
  const fetchClientData = async (id?: string) => {
    ...
    try {
      ...
      const { data: programs } = await supabase
        .from('programs')
        .select('id, name, status, scheduled_date, type')
        .eq('assigned_to', id)
        .eq('coach_id', user.id)
      ...
      const { data: checkinHistory } = await supabase
        .from('daily_logs')
        .select('mood, energy, stress, sleep_quality, created_at')
        .eq('user_id', id)
      ...
      const fitnessProgram = programs?.find(p => p.type === 'fitness');
      const nutritionProgram = programs?.find(p => p.type === 'nutrition');
      const mentalHealthProgram = programs?.find(p => p.type === 'mental_health');
      ...
      setClientData({
        ...
        programs: (programs || []).map(p => ({
          ...
          type: p.type as 'fitness' | 'nutrition' | 'mental_health',
        })),
        adherence,
        programDays: {
          total: totalDays,
        ...
        dailyCheckin: {
          ...
          trend: {
            ...
            sleep: calculateTrend('sleep_quality'),
          },
        },
        weightJourney: {
